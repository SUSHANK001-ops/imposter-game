...
export const WordBankModal: React.FC<WordBankModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="glass-panel w-full max-w-lg rounded-2xl border border-purple-800/50 p-6 shadow-2xl relative overflow-hidden">
        {/* Close Button */}
        <button
          onClick={onClose}
        ...
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h2 className="text-lg font-bold text-white">Admin Word Bank & Custom Words</h2>
        ...
        <div className="mb-6">
          <label className="text-xs font-semibold text-purple-300 mb-2 block">Active Categories ({SEED_DATA.length}):</label>
          <div className="flex flex-wrap gap-1.5 max-h-28 overflow-y-auto p-2 bg-slate-900/60 rounded-xl border border-purple-950/60">
            {SEED_DATA.map((cat, i) => (
              <span key={i} className="text-[11px] px-2.5 py-1 rounded-lg bg-purple-950/80 border border-purple-800/40 text-purple-200">
        ...
        {/* Add Word Form */}
        <form onSubmit={handleAddWord} className="space-y-4 bg-slate-950/50 p-4 rounded-xl border border-purple-900/40">
          <h3 className="text-xs font-bold uppercase tracking-wider text-purple-400">Add Custom Secret Word</h3>
          ...
          <div className="space-y-3">
            <div>
              ...
              <input
                ...
                className="w-full bg-slate-900 border border-purple-900/60 rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus:border-purple-500"
              />
            </div>
          ...
          <button
            type="submit"
            className="w-full bg-purple-600 hover:bg-purple-500 text-white font-medium text-xs py-2.5 rounded-xl transition-all flex items-center justify-center gap-2 shadow-lg"
          >
